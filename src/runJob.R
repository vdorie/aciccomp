...
for (i in seq_along(iters)) {
  ...
  if (!is.null(attr(consoleLog, "status"))) {
    ...
    write.csv(result, file = outFile, sep = ",", dec = ".", row.names = FALSE, col.names = method$headers_out == 1L)
    ...
    if (method$individual_effects != 0L) {
      ...
      write.csv(result, file = outFile.ind, sep = ",", dec = ".", row.names = FALSE, col.names = method$headers_out == 1L)
    }
  } else {
